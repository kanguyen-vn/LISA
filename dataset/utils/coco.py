...
class COCO:
    def __init__(self, annotation_file=None, areaThreshold=None, verbose=False):
        """
    ...
    def createIndex(self):
        ...
        if "annotations" in self.dataset:
            for ann in self.dataset["annotations"]:
                ...
                if "area" in ann:
                    ratio = ann["area"] / (img["width"] * img["height"])
                    if self.areaThreshold is not None and ratio < self.areaThreshold:
                        continue
                    ann["ratio"] = ratio
                imgToAnns[ann["image_id"]].append(ann)
                anns[ann["id"]] = ann

            imgs = {k: v for k, v in imgs.items() if k in imgToAnns}
