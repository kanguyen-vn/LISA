...
from dataset.utils.utils import SEG_ANSWER_LIST, SEG_QUESTIONS
...
class ReferSegmDataset(torch.utils.data.Dataset):
    ...
    def initialize_refer_segm_data(self, refer_segm_data, inference=False):
        ...
        for dataset_name in self.refer_seg_ds_list:
            ...
            print(
                f'\033[92m----SEG-{"Val" if self.validation else "Train"}:'
    ...
    def load_images(
        self, refer_api, images_ids_train, dataset_dir, dataset_name, inference=False
    ):
        images = []
        ...
        loaded_images = (
            loaded_images[:1000]
            if (self.validation and not inference)
            else loaded_images
        )
        for item in loaded_images:
            ...
            if dataset_name == "refclef":
                ...
                item["file_name"] = os.path.join(
                    dataset_dir.replace("Refer_Segm/", ""),
                    "coco_2014/train2014",
                    item["file_name"],
                )
            images.append(item)
    ...
    def create_conversations(self, labels):
        ...
        for i, label in enumerate(labels):
            ...
            # answers.append(answer_template.format(class_name=label.lower()))
    ...
    def __getitem__(self, idx):
        ...
        idx = (
            idx
            if (self.validation or not self.random_sampling)
            else random.randint(0, len(images) - 1)
        )
        ...
        image = cv2.imread(image_path)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        # global_enc_img = self.global_enc_processor.preprocess(
        #     image, return_tensors="pt"
        # )["pixel_values"][0]
        global_enc_img = self.global_enc_process(image_path)
        image = self.transform.apply_image(image)
        image_resize = image.shape[:2]
        grounding_enc_img = self.grounding_enc_processor(
