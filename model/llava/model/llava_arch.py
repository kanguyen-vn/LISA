...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_paired_inputs_labels_for_multimodal(
        self,
        input_ids,
        attention_mask,
        past_key_values,
        ...
        for batch_idx, (cur_input_ids, reg_feat) in enumerate(
            ...
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0:
                ...
                if labels is not None:
                    new_labels.append(labels[batch_idx])
                cur_image_idx += 1
                image_token_slices.append([])
                continue
            image_token_indices = torch.where(cur_input_ids == IMAGE_TOKEN_INDEX)[0]
            cur_new_input_embeds = []
            per_input_ids_image_token_slices = []
            if labels is not None:
                cur_labels = labels[batch_idx]
                cur_new_labels = []
                assert cur_labels.shape == cur_input_ids.shape
            while image_token_indices.numel() > 0:
                cur_image_features = image_features[cur_image_idx]
                # print(f"{cur_image_features.shape = }")
                image_token_start = image_token_indices[0]
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    self.config, "mm_use_im_start_end", False
                ):
                    ...
                    image_embed_start_idx = sum(
                        input_embeds_each.shape[0]
                        for input_embeds_each in cur_new_input_embeds
                    )
                    image_embed_end_idx = (
                        image_embed_start_idx + cur_image_features.shape[0]
                    )
                    per_input_ids_image_token_slices.append(
                        slice(image_embed_start_idx, image_embed_end_idx)
                    )
                    cur_new_input_embeds.append(cur_image_features)
                    cur_new_input_embeds.append(
                        self.get_model().embed_tokens(
                ...
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    ...
                    cur_input_ids = cur_input_ids[image_token_start + 1 :]
                image_token_indices = torch.where(cur_input_ids == IMAGE_TOKEN_INDEX)[0]
            image_token_slices.append(per_input_ids_image_token_slices)
            if cur_input_ids.numel() > 0:
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    self.config, "mm_use_im_start_end", False
                ):
                ...
                if labels is not None:
                    cur_new_labels.append(cur_labels)
            # print(f"{[x.shape for x in cur_new_input_embeds] = }")
            cur_new_input_embeds = [
                x.to(device=self.device) for x in cur_new_input_embeds
            ]
            cur_new_input_embeds = torch.cat(cur_new_input_embeds, dim=0)
