...
def collate_fn_multi(
    ...
    for (
        ...
        conversation_list.extend(conversations)
        label_list.append(label)
        masks_list.append([masks_each.float() for masks_each in masks])
        resize_list.append(resize)
        questions_list.append(questions)
        sampled_classes_list.append(sampled_classes)
        cnt += len(conversations)
        ...
        image_offset_list.append(image_cnt)
        inferences.append(inference)
    ...
    if inferences[0] == False:
        max_image_count = max(
            [
                image_offset_list[i + 1] - image_offset_list[i]
                for i in range(len(image_offset_list) - 1)
            ]
        )
        truncate_len = tokenizer.model_max_length - (256 - 1) * max_image_count
    ...
    return {
        ...
        "resize_list": resize_list,
        "offset": torch.LongTensor(offset_list),
        "image_offset": torch.LongTensor(image_offset_list),
        "questions_list": questions_list,
        "sampled_classes_list": sampled_classes_list,
        "inference": inferences[0],
        "conversation_list": conversation_list,
